...
const addPermission = (req, res) => {
  ...
  const checkPermissionQuery =
    "SELECT * FROM UserPermissions WHERE user_id = ? AND permission_id IN (SELECT id FROM Permissions WHERE name = ?)";
  const checkPermissionData = [user_id, name];

  connection.query(
    checkPermissionQuery,
    checkPermissionData,
    (checkPermissionErr, checkPermissionResult) => {
      if (checkPermissionErr) {
        console.log(checkPermissionErr);
        return res
          .status(500)
          .json({ message: "Failed to check user permission" });
      }
      ...
      connection.query(
        addPermissionQuery,
        addPermissionData,
        (addPermissionErr, addPermissionResult) => {
          if (addPermissionErr) {
            return res
              .status(500)
              .json({ message: "Failed to add permission" });
          }

          const permission_id = addPermissionResult.insertId;

          const addUserPermissionQuery =
            "INSERT INTO UserPermissions (permission_id, user_id) VALUES (?, ?)";
          const addUserPermissionData = [permission_id, user_id];

          connection.query(
            addUserPermissionQuery,
            addUserPermissionData,
            (addUserPermissionErr, addUserPermissionResult) => {
              if (addUserPermissionErr) {
                return res
                  .status(500)
                  .json({ message: "Failed to add user permission" });
              }
